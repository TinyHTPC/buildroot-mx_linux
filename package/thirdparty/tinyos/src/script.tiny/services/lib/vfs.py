...
APP_KEY = backuputils.getSetting('dropbox_key')
APP_SECRET = backuputils.getSetting('dropbox_secret')
...
class DropboxFileSystem(Vfs):
    ...
    def setup(self):
        if(APP_KEY == '' or APP_SECRET == ''):
            xbmcgui.Dialog().ok(backuputils.getString(30010),backuputils.getString(30058),backuputils.getString(30059))
            return
        ...
        if(user_token_key == '' and user_token_secret == ''):
            ...
            #print url in log
            backuputils.log("Authorize URL: " + url)
            xbmcgui.Dialog().ok(backuputils.getString(30010),backuputils.getString(30056),backuputils.getString(30057))  
        ...
        try:
            backuputils.log(str(self.client.account_info()))
        except:
    ...
    def setToken(self,key,secret):
        ...
        token_file = open(xbmc.translatePath(backuputils.data_dir() + "tokens.txt"),'w')
        token_file.write("%s|%s" % (key,secret))
        token_file.close()

    def getToken(self):
        #get tokens, if they exist
        if(xbmcvfs.exists(xbmc.translatePath(backuputils.data_dir() + "tokens.txt"))):
            token_file = open(xbmc.translatePath(backuputils.data_dir() + "tokens.txt"))
    ...
    def deleteToken(self):
        if(xbmcvfs.exists(xbmc.translatePath(backuputils.data_dir() + "tokens.txt"))):
            xbmcvfs.delete(xbmc.translatePath(backuputils.data_dir() + "tokens.txt"))
