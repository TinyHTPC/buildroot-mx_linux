...
import xbmcgui
import xbmcvfs
import backuputils as backuputils
import os.path
import time
...
class XbmcBackup:
    ...
    def __init__(self):
        ...
        backuputils.log(backuputils.getString(30046))

    def configureRemote(self):
        if(backuputils.getSetting('remote_selection') == '1'):
            self.remote_base_path = backuputils.getSetting('remote_path_2');
            self.remote_vfs = XBMCFileSystem(backuputils.getSetting('remote_path_2'))
	    backuputils.setSetting("remote_path","")
        elif(backuputils.getSetting('remote_selection') == '0'):
            self.remote_base_path = backuputils.getSetting('remote_path');
            self.remote_vfs = XBMCFileSystem(backuputils.getSetting("remote_path"))
        elif(backuputils.getSetting('remote_selection') == '2'):
    ...
    def run(self,mode=-1,runSilent=False):
        ...
        progressBarTitle = backuputils.getString(30010) + " - "
        if(mode == self.Backup and self.remote_vfs.root_path != ''):
            self.remote_vfs.set_root(self.remote_vfs.root_path + time.strftime("%Y%m%d%H%M") + "/")
            progressBarTitle = progressBarTitle + backuputils.getString(30016)
	    ...
	    progressBarTitle = progressBarTitle + backuputils.getString(30017)
	else:
            #kill the program here
	    self.remote_vfs = None
	    return

        backuputils.log(backuputils.getString(30047) + ": " + self.xbmc_vfs.root_path)
        backuputils.log(backuputils.getString(30048) + ": " + self.remote_vfs.root_path)

        #check if we should use the progress bar
        if(backuputils.getSetting('run_silent') == 'false' and not runSilent):
            self.progressBar = xbmcgui.DialogProgress()
            self.progressBar.create(progressBarTitle,backuputils.getString(30049) + "......")

        if(mode == self.Backup):
            backuputils.log(backuputils.getString(30023) + " - " + backuputils.getString(30016))
            #check if remote path exists
            if(self.remote_vfs.exists(self.remote_vfs.root_path)):
                #may be data in here already
                backuputils.log(backuputils.getString(30050))
            else:
            ...
            backuputils.log(backuputils.getString(30051))
            allFiles = []
            fileManager = FileManager(self.xbmc_vfs)
         
            #go through each of the user selected items and write them to the backup store
            if(backuputils.getSetting('backup_addons') == 'true'):
            ...
            if(backuputils.getSetting('backup_addon_data') == 'true'):
            ...
            if(backuputils.getSetting('backup_database') == 'true'):
            ...
            if(backuputils.getSetting("backup_playlists") == 'true'):
            ...
            if(backuputils.getSetting('backup_profiles') == 'true'):
            ...
            if(backuputils.getSetting("backup_thumbnails") == "true"):
            ...
            if(backuputils.getSetting("backup_config") == "true"):
            ...
            if(backuputils.getSetting('custom_dir_1_enable') == 'true' and backuputils.getSetting('backup_custom_dir_1') != ''):

                #create a special remote path with hash                
                self.xbmc_vfs.set_root(backuputils.getSetting('backup_custom_dir_1'))
            ...
            if(backuputils.getSetting('custom_dir_2_enable') == 'true' and backuputils.getSetting('backup_custom_dir_2') != ''):

                #create a special remote path with hash                
                self.xbmc_vfs.set_root(backuputils.getSetting('backup_custom_dir_2'))
                self.remote_vfs.mkdir(self.remote_vfs.root_path + "custom_" + self._createCRC(self.xbmc_vfs.root_path))
            ...
            backuputils.log(backuputils.getString(30023) + " - " + backuputils.getString(30017))

            #for restores remote path must exist
            if(not self.remote_vfs.exists(self.remote_vfs.root_path)):
                xbmcgui.Dialog().ok(backuputils.getString(30010),backuputils.getString(30045),self.remote_vfs.root_path)
                return
            ...
            if(backuputils.getSetting("backup_config") == "true"):
                ...
                if(self.remote_vfs.exists(self.remote_vfs.root_path + "userdata/advancedsettings.xml") and not self.skip_advanced):
                    ...
                    restartXbmc = xbmcgui.Dialog().yesno(backuputils.getString(30038),backuputils.getString(30039),backuputils.getString(30040), backuputils.getString(30041))
                    ...
                    if(restartXbmc):
                        ...
                        xbmcgui.Dialog().ok(backuputils.getString(30077),backuputils.getString(30078))
            ...
            if(backuputils.getSetting('custom_dir_1_enable') == 'true' and backuputils.getSetting('backup_custom_dir_1') != ''):
                ...
                if(self.remote_vfs.exists(self.remote_vfs.root_path + "custom_" + self._createCRC(self.xbmc_vfs.root_path))):
                    ...
                    xbmcgui.Dialog().ok(backuputils.getString(30010),backuputils.getString(30045),self.remote_vfs.root_path + "custom_" + self._createCRC(backuputils.getSetting('backup_custom_dir_1')))
            ...
            if(backuputils.getSetting('custom_dir_2_enable') == 'true' and backuputils.getSetting('backup_custom_dir_2') != ''):
                ...
                if(self.remote_vfs.exists(self.remote_vfs.root_path + "custom_" + self._createCRC(self.xbmc_vfs.root_path))):
                    ...
                    xbmcgui.Dialog().ok(backuputils.getString(30010),backuputils.getString(30045),self.remote_vfs.root_path + "custom_" + self._createCRC(backuputils.getSetting('backup_custom_dir_2')))
    ...
    def backupFiles(self,fileList,source,dest):
        backuputils.log("Writing files to: " + dest.root_path)
        backuputils.log("Source: " + source.root_path)
        for aFile in fileList:
            if(not self._checkCancel()):
                backuputils.log('Writing file: ' + aFile,xbmc.LOGDEBUG)
    ...
    def _rotateBackups(self):
        total_backups = int(backuputils.getSetting('backup_rotation'))
        if(total_backups > 0):
            ...
            if(len(dirs) > total_backups):
                ...
                while(remove_num < (len(dirs) - total_backups) and not self._checkCancel()):
                    self._updateProgress(backuputils.getString(30054) + " " + dirs[remove_num][1])
                    backuputils.log("Removing backup " + dirs[remove_num][0])
                    self.remote_vfs.rmdir(self.remote_base_path + dirs[remove_num][0] + "/")
                    remove_num = remove_num + 1

    def _createValidationFile(self):
        vFile = xbmcvfs.File(xbmc.translatePath(backuputils.data_dir() + "xbmcbackup.val"),'w')
        vFile.write("XBMC Backup Validation File")
        vFile.close()

        self.remote_vfs.put(xbmc.translatePath(backuputils.data_dir() + "xbmcbackup.val"),self.remote_vfs.root_path + "xbmcbackup.val")

    def _createResumeBackupFile(self):
        rFile = xbmcvfs.File(xbmc.translatePath(backuputils.data_dir() + "resume.txt"),'w')
...
class FileManager:
    ...
    def addFile(self,filename):
        ...
        backuputils.log("Add File: " + filename,xbmc.LOGDEBUG)
